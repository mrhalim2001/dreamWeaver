...
def craft_image_prompt(story_json):
    ...
    prompt = "You are a kids storybook designer and your job is not imagine an image for children storybook cover page. Read the following story then imagine a suitable cover image. Output a one-sentence  description of the image containing at least 50 words, detailing its visual components in writing. The sentence must start with \"A colorful cartoonish image showing \". Avoid using proper names, using instead detailed visual descriptions of the subjects.\n\nHere is the story text:\n\n"
...
def invent_an_image(image_prompt):
    ...
    image_1_b64_json = response.data[0].b64_json
    image_1_data = base64.b64decode(image_1_b64_json)
    image1 = Image.open(BytesIO(image_1_data))

    return image1, image_1_data
